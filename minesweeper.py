...
class MinesweeperAI():
    ...
    def __init__(self, height=10, width=10, mine=16):
    ...
    def make_random_move(self):
        ...
        # Otherwise can potentially improve random choice using KB:
            ...
            for sentence in self.knowledge:
                ...
                # Update mine probabilty of each cell :
                for cell in sentence.cells:
                    moves[cell] += mine_prob
            for i in range(0, self.height):
                for j in range(0, self.width):
                    if (i, j) not in self.moves_made and (i, j) not in self.mines and moves[(i, j)] == 0 :
                        not_in_sentence+=1
            for i in range(0, self.height):
                for j in range(0, self.width):
                    if (i, j) not in self.moves_made and (i, j) not in self.mines and moves[(i, j)] == 0 :
                        moves[(i, j)] = (num_mines_left - all_count)/not_in_sentence
